...
func (s *FileTokenStore) saveWithDirectorySync(ctx context.Context, auth *cliproxyauth.Auth, imported bool, syncDirectory func(string) error) (string, error) {
	...
	// was deliberately removed. Explicit imports and login or migration callers
	// with creation intent may intentionally create a missing disabled credential.
	if auth.Disabled && !imported && !cliproxyauth.HasAuthCreationIntent(ctx) {
